...
class SitePart extends Model
{
    ...
    protected $maps = [
      ...
      // simple alias
        ...
        'qty'                   => 'QUANTITY',
        'is_ticket'             => 'TESTCUTS',
        'is_ticket_weekday'     => 'ISTIXWEEKDAY'
    ...
    public function part(){
        return $this->belongsTo('App\Part','sitepart_id');
    }
    
    public function sitepart(){
        return $this->belongsTo('App\Model\OrderSlipDetail');
    }
    public function withComponent(){
        return static::where('product_id','product_id')
        ->where('display',1);
    }
    public function withoutComponent(){
        return static::where('product_id','product_id')
            ->where('modifiable',0)
            ->where('display',1);
    }
